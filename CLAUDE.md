...
OAuth, Supabase backend (9 Edge Functions, migrations 0001–0032), self-healing shared CC0
...
## Multi-user — SHIPPED (v1.0.0 launched 2026-08-29; current v1.23.2). Social waves shipped through friend-crate visibility (#43), any-pressing matching (#28), the selling wave (for-sale via Discogs, v1.16–1.19) + Wave 5a "share the shelf" (Collection DNA card + shareable filter URLs, v1.20.0). Recent UI: FILED UNDER style tray (v1.21.0), FOR SALE promoted to a 5th tab (v1.22.0), header icon trio (v1.23.x). End-of-phase cold audit done (v1.14.0)
...
uuid). Migrations run **0001–0032**; highlights: 0012–0015 friendship graph + friend-read RLS +
...
(selling), 0028 `forsale_visibility` + `get_friend_forsale`, 0031 `list_friends`, 0032 `master_year`. Auth is
