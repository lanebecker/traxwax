-- 0038_public_relation_first.sql — Wave 5b S1 remediation-audit F4.
-- get_public_crate computed relation AFTER the all-private gate, so a FRIEND following a
-- circulated /c/ link after the owner reverted to friends-only got a hard 404 instead of the
-- /app/{handle} redirect their friend tier still honors — and the OWNER of a dead slug needed a
-- client-side profile probe to see the CLOSED page. Reorder: relation first. Strangers are
-- unchanged (all-private ≡ unknown, NULL either way); the owner's redirect now carries
-- 'open' so the client renders CLOSED directly. Body otherwise identical to 0037.
