...
RESTRICTED_KEYS = ('price', 'crating', 'crcount', 'have', 'want')   # A2 (#63)
...
def main():
    ...
    if SKIP_RELEASES:
        ...
        # A2 (#63): only records MISSING their static file need a get_release call — the
        # payload is immutable and the mutable-stat refresh is gone.
    ...
    # A2 (#63) hard guard: refuse to write a fixture carrying any Restricted key. This is
    # the last line of defense against a future edit re-introducing them.
    leaked = sorted({k for r in records for k in RESTRICTED_KEYS if k in r})
    if leaked:
        print(f'ERROR: Restricted key(s) {leaked} present in output — refusing to write.',
              file=sys.stderr)
        sys.exit(1)

    json.dump(records, open(OUT, 'w'), ensure_ascii=False, separators=(',', ':'))
    print(f'Wrote {len(records)} records to collection.json (CC0 fields only)')
