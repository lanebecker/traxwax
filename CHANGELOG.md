...
### Fixed
- **Sign-up/sign-in card no longer overflows the viewport on phones (#107).** Clerk's
  `.cl-cardBox` ships a fixed width (350px mobile variant) that ignores its container; at a
  390px viewport the state card's content slot is ~295px, so the whole form — Google button,
  inputs, CONTINUE — poked ~55px past the card and forced horizontal scroll. The `.cl-*`
  block now caps the Clerk frame at `max-width:100%`; desktop slots exceed Clerk's width, so
  desktop renders pixel-identical (verified live with the rule toggled at 1280px).
- **Dark theme: "Continue with Google" label readable again (#108).** The button text was
  Clerk's own black at 62% alpha on the `#17181b` card — near-invisible. It now inherits
  `var(--ink)` and follows the live theme.
